...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, n_channels=3, num_classes=1, final_pool_dim=(1,1), concat_features=True, concat_data_len=1,
                 use_dropout=False, zero_init_residual=False, groups=1, 
        ...
        self.avgpool = nn.AdaptiveAvgPool2d(final_pool_dim)        
        if self.use_dropout:
            self.dropout = nn.Dropout(0.5, inplace=True)
        
#         last_len = 15
#         self.fc1 = nn.Linear(512 * block.expansion, last_len)
#         if self.concat_features:
#             self.fc2 = nn.Linear(last_len + concat_data_len, 8)
#             last_len = 8
        
        last_len = 5
        self.fc1 = nn.Linear(512 * block.expansion, last_len)
        if self.concat_features:
            self.fc2 = nn.Linear(last_len + concat_data_len, 4)
            last_len = 4
        
        self.fc3 = nn.Linear(last_len, num_classes)
...
def resnet_tiny(pretrained=False, progress=True, **kwargs):
